...
P = []
CROPS = ["entropy", "top", "left", "right", "bottom", "center"]
def add(cat, name, price, img, desc, tag=None, stock=12):
    n = len(P); crop = CROPS[n % len(CROPS)]; f = (n * 7) % 6
    P.append({"id": f"p{n+1:02d}", "cat": cat, "name": name, "price": price, "img": IMG[img] + "&crop=" + crop, "f": f, "desc": desc, "tag": tag, "stock": [12, 3, 12, 5, 12, 12, 2][n % 7]})
...
.p img{aspect-ratio:1;object-fit:cover;width:100%}.f1{filter:hue-rotate(-14deg) saturate(1.15)}.f2{filter:sepia(.4) contrast(1.05)}.f3{filter:contrast(1.12) brightness(.93)}.f4{filter:hue-rotate(18deg) saturate(.9)}.f5{filter:grayscale(.35) contrast(1.08)}.p .b{padding:14px 14px 16px;display:flex;flex-direction:column;gap:5px;flex:1}
...
(function(){
  ...
  function card(p){var w=g(K.wish,[]).indexOf(p.id)>=0; return '<div class="p">'+(p.tag?'<span class="badge">'+p.tag+'</span>':'')+'<button class="wish'+(w?' on':'')+'" data-w="'+p.id+'" aria-label="wishlist">'+(w?'♥':'♡')+'</button><a href="product.html?id='+p.id+'"><img class="f'+p.f+'" src="'+p.img+'" alt="" loading="lazy"></a><div class="b"><h3><a href="product.html?id='+p.id+'">'+p.name+'</a></h3><div class="d">'+p.desc+'</div>'+(p.stock<=5?'<div class="low">Only '+p.stock+' left</div>':'')+'<div class="pr"><b>'+money(p.price)+'</b><button class="add" data-id="'+p.id+'">Add</button></div></div></div>';}
