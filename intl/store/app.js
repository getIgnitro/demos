...
(function(){
  ...
  function render(){
    var grid=document.getElementById('products'); if(grid){
      grid.innerHTML=P.filter(function(p){return filter==='all'||p.cat===filter;}).map(function(p){return '<div class="p">'+(p.badge?'<span class="badge">'+p.badge+'</span>':'')+'<img src="'+p.img+'" alt=""><div class="b"><h3>'+(ar()?p.ar:p.en)+'</h3><div class="d">'+(ar()?p.dar:p.den)+'</div><div class="pr"><b>$'+p.price+'</b><button class="add" data-id="'+p.id+'">'+(ar()?'أضف':'Add')+'</button></div></div></div>';}).join('');
    ...
    var cart=document.getElementById('cart'); if(cart){
      ...
      s('sub','$'+sub); s('del',del?'$'+del:(ar()?'مجانًا':'Free')); s('tot','$'+tot);
